...
*/
public class EditorTestPageItem extends MdPageMenuItem {

    public EditorTestPageItem(Dictionary dictionary, Stage stage, TabFactory tabFactory) {
        super(dictionary.TOOLBAR_EDITOR_OPEN_TEST_PAGE_ITEM);
    ...
    public EventHandler<ActionEvent> getClickAction(final Dictionary dictionary, final Stage stage, final TabFactory tabFactory) {
        return event -> {
            EditorPane editorPane = new EditorPane(dictionary, Utils.getTestText());
            EditorTab newTab = new EditorTab(editorPane, stage);
            newTab.getEditorPane().setFile(new File(Utils.getTestPageFileName()));
